...
Human::Human(ProfessionsType type, string name, string lastname, short age, short weight, short height)
{
    setlocale(LC_ALL, "ru_RU.UTF-8");
    //setlocale(LC_ALL, "ru_RU");
...
Human::Human(const std::string name, short age)
{
...
std::ostream &operator<<(std::ostream &output, const Human &m) {
    ...
    output << "Вес: " << m.weight << "\n";
    output << "Профессия: " << ((m.type == ProfessionsType::STUDENT) ? "Студент\n" : "Не студент\n");
    return output;
}

// Создать пользователя
void Human::createUsers(const std::string n, const std::string ln, short a, short w, short h)
{
    name = n;
    lastname = ln;
    age = a;
